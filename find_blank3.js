...
async function run() {
    const { data, error } = await supabase.from('projects').select('id, project_id, client_name, project_title');
    console.log("All projects:", data.filter(p => !p.project_id || p.project_id.trim() === ''));
}
run();
