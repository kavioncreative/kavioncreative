...
async function run() {
    const { data, error } = await supabase.from('projects').select('id, project_id, client_name, project_title').order('created_at', { ascending: false }).limit(10);
    console.log("Last 10 projects:", data);
}
run();
