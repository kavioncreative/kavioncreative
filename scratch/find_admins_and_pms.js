...
async function run() {
    ...
    const rolesMap = {};
    profiles.forEach(p => {
        rolesMap[p.role] = (rolesMap[p.role] || 0) + 1;
    });
    console.log('Roles distribution:', rolesMap);

    console.log('\n--- Admins/PMs/Super Admins: ---');
    const privileged = profiles.filter(p => 
        ['super admin', 'admin', 'project manager', 'project operations manager'].includes(p.role?.toLowerCase())
    );
