...
async function checkProject() {
    console.log('Checking project MAN 720429...');
    const { data, error } = await supabase
        .from('projects')
        .select('*')
        .eq('logo_no', 'MAN 720429');
    ...
    if (data.length === 0) {
        console.log('Project not found with logo_no MAN 720429');
        return;
    }

    console.log('Project Data:', JSON.stringify(data[0], null, 2));
}
