...
export function Table<T>({
  ...
  const isTableDense = dense || isDense;
            ...
            >
              {columns.map((col, idx) => {
                ...
                return (
                  <th
                    key={idx}
                    className={`${isTableDense ? 'px-4 py-2 text-[10px]' : 'px-6 py-4 text-[11px]'} font-bold uppercase tracking-widest whitespace-nowrap ${isMetallicHeader ? 'text-white' : 'text-gray-400'} ${headerAlignmentClasses}`}
                  >
                    {col.header}
                  </th>
                );
              ...
              [...Array(skeletonCount)].map((_, rowIdx) => (
                <tr key={rowIdx}>
                  {columns.map((col, colIdx) => {
                    ...
                    return (
                      <td key={colIdx} className={`${isTableDense ? 'px-4' : 'px-6'} ${col.className || ''}`} style={{ height: isTableDense ? '40px' : '58px' }}>
              ...
              data.map((item, rowIdx) => {
                ...
                return (
                  <tr
                    ...
                    style={{ height: isTableDense ? '40px' : '58px' }}
                  >
                    {columns.map((col, colIdx) => {
                      if (skipRemaining > 0) {
                        skipRemaining--;
                      ...
                      return (
                        <td 
                          key={colIdx} 
                          colSpan={span}
                          className={`${isTableDense ? 'px-3 py-2 text-xs' : 'px-6 py-3 text-sm'} text-gray-300 ${colIdx % 2 === 1 && span === 1 ? 'bg-white/[0.02]' : ''} ${col.className || ''}`}
                        >
